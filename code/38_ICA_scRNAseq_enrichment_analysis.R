...
if (FALSE){
  setwd("/Volumes/broad_sankaranlab/ebao/HCA/scSVA/examples/ICA_BM_Seurat/")
  setwd("/broad/sankaranlab/ebao/HCA/scSVA/examples/ICA_BM_Seurat/")
  ...
  mpn <- read.table("../output/target_genes/target_genes_combined.PP10.txt")[,1] %>% as.character()
  mpn <- read.table("/broad/sankaranlab/MPN_GWAS/metaGWAS/target_genes/target_genes_combined.PP10.txt")[,1] %>%
    as.character()
  ...
  fwrite(merged,file="mpn_hsc_signatures.tsv",sep="\t")
}
...
# Looking only at cells which express 1 or more genes in HSC and MPN signature
detected <- merged %>% filter(mpn_signature > min(mpn_signature)) 
cor.test(detected$hsc_signature,detected$mpn_signature,method="spearman")
ggplot(detected,aes(x=hsc_signature,y=mpn_signature))+ 
  geom_point()+
...
if (FALSE){
  # Not a normal distribution, can't do pearson correlation
  shapiro.test(merged$hsc_signature)
  cor.test(merged$hsc_signature,merged$mpn_signature,method="pearson")
  
  # Min max
  minmax <- function(x) return((x- min(x)) /(max(x)-min(x)) * 100)
  merged$hsc_minmax <- minmax(merged$hsc_signature)
  merged$mpn_minmax <- minmax(merged$mpn_)
...
# ggplot(longer,aes(y=cluster,x=score_capped,fill=geneset))+
#   geom_density_ridges()+
#   scale_fill_manual(values = jdb_palette("brewer_spectra")[c(2,3)]) +
#   geom_vline(xintercept=median(longer$score_capped),linetype=2)+
#   pretty_plot(fontsize=8) + L_border() +
#   labs(x="Score",y="Gene set")

# Group by 12,28,17 vs. all others
...
ggplot(longer,aes(y=geneset,x=score_capped,fill=HSC_binary))+
  geom_density_ridges()+
  scale_fill_manual(values = jdb_palette("brewer_spectra")[c(2,3)]) +
  pretty_plot(fontsize=8) + L_border() +
  labs(x="Score",y="Gene set")

ggplot(merged,aes(x=hsc_signature,y=mpn_signature))+
  geom_point()+
  pretty_plot(fontsize=8)+ L_border()
