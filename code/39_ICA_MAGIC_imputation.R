...
h5file <- "/Volumes/broad_sankaranlab/ebao/HCA/scSVA/examples/ICA_BM_Seurat/ICA_BM.h5"
...
# Normal distribution?
if (FALSE){
  plot(density(merged$hsc_signature))
  plot(density(merged$mpn_signature))
  qqnorm(merged$mpn_signature);qqline(merged$mpn_signature, col = 2)
  summary(lm(mpn_signature ~ hsc_signature,data=merged))
}
