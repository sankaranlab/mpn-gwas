...
# tg_cpm <- log2cpm.minmax[rownames(log2cpm.minmax) %in% target_genes$V1,]

# Cluster by ATAC and plot heatmap
km <- kmeans(tg_cpm, centers = 5, nstart = 10000) 
km.cluster <- factor(km$cluster, levels = c("1","3","4","5","2"))

pdf(file=paste0("../output/target_genes/target_genes_expression_heatmap_all_CHEK2_included.pdf"), width = 4, height = 4)
...
cowplot::ggsave2(p1,file="../output/target_genes/target_genes_expression_heatmap_gap.pdf",height=2,width=2)

# Percentile analysis of target genes
counts.normalized<- normalize.quantiles(as.matrix(counts))
ecdf(counts.normalized[,1])(counts.normalized[target_genes[1],1])
